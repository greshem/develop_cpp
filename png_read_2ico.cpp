 ...
 * body data.
 */
static struct ico_fragment * read_png(const char *file)
{
	...
	frag = (ico_fragment  *)zalloc(sizeof(*frag));
	...
	fprintf(stderr, "only support < 255*255 png file\n", file);
	goto errexit;
	}
	...
	row_pointers =  (uint8_t **)xmalloc(sizeof(uint8_t *) * height);
	for ( y = 0 ; y < height ; y++ ) 
	{
	row_pointers[y] = (uint8_t * ) xmalloc(width);
	}
	...
	frag->data  = data  =     (uint8_t*) zalloc(datasize  = dwidth*height);
	frag->data_size = datasize;
	frag->alpha = alpha = (uint8_t*)zalloc(alphasize = awidth*height);
	frag->alpha_size = alphasize;

	hdr->colorcount = max_colors & 0xff;
	frag->pal = pal = (ico_color*)zalloc(max_colors * sizeof(*pal));
	frag->pal_size = max_colors * sizeof(*pal);

	write_le32(hdr->sizeinbytes,sizeof(*info)+frag->pal_size+datasize+alphasize);
