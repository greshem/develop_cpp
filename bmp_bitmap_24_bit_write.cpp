...
int dump_data_to_file( int *data, int width , int height , char *filename )
{
   	//   SaveBitmap("test.bmp");
	...
	printf(" header size = %d", sizeof(header));
	assert(ret==14);
...
int main()
{
	...
	for( int i=0; i<width; i++)
	{
		for(int j=0; j<height; j++)
		{
			...
			{
				data[j*width +i]=0xffffff;
			}
		}
	} 
	//dump
	dump_data_to_file(data, width, height, "output.bmp");	
}
