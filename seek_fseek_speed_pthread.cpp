...
typedef struct {
	...
	off64_t maxoffset;
	off64_t minoffset;
	long seek_limit; //假如是一个文件而不是磁盘的话随机seek 会出现错误。 
} parm;
...
int main(int argc, char **argv) 
{
	...
	printf("Seeker v3.0, 2009-06-17, "  "http://www.linuxinsight.com/how_fast_is_your_disk.html\n");
	...
	std::cout<<argv[1]<<" size is "<< filesize<<std::endl;
#ifdef BLKGETSIZE64
	...
	for (i = 0; i < threads; i++) 
	{
		...
		p[i].fd = dup(fd);
		handle("dup", p[i].fd < 0);
		p[i].buffer = (char *)malloc(sizeof(char)*BLOCKSIZE);
