...
typedef unsigned char   s8;                     /* Signed types of an exact size */
typedef short  s16;
typedef int  s32;
...
int ntfs_boot_sector_dump(struct ntfs_boot_sector in )	
{
	...
	cout<< "u8	sectors_per_cluster		"<< (int)in.sectors_per_cluster<<endl;
	...
	cout<< "int_64	mftmirr_lcn		"<< (unsigned long long  )in.mftmirr_lcn<<endl;
	cout<< "s8	clusters_per_mft_record		"<< (unsigned  )in.clusters_per_mft_record<<endl;
	cout<< "u8	reserved0[3]		"<< in.reserved0[3]<<endl;
	cout<< "s8	clusters_per_index_record		"<< (int)in.clusters_per_index_record<<endl;
...
int main(int argc, char *argv[])
{
	...
	if(argc != 2)
	{
		printf("Usage: %s /dev/ntfs_device", argv[0]);
		exit(-1);
	}
	file=fopen(argv[1], "r");
	if(file==NULL)
