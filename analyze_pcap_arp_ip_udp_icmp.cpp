...
//判断文件是否结束
	    ...
	    printf("capture time is: %d.%d\n", (*pkt_p).ts.tv_sec, (*pkt_p).ts.tv_usec);
	    printf("caplen is:%d bytes\n", (*pkt_p).caplen);
	    printf("len is:%d bytes\n", (*pkt_p).len);
	    fread(eth_p, sizeof(struct EthernetPacket), 1, fp);
	    printf("destination mac address:%02X:%02X:%02X:%02X:%02X:%02X\n", (*eth_p).MacDst[0], (*eth_p).MacDst[1], (*eth_p).MacDst[2], (*eth_p).MacDst[3], (*eth_p).MacDst[4], (*eth_p).MacDst[5]);
	    printf("source mac address:%02X:%02X:%02X:%02X:%02X:%02X\n", (*eth_p).MacSrc[0], (*eth_p).MacSrc[1], (*eth_p).MacSrc[2], (*eth_p).MacSrc[3], (*eth_p).MacSrc[4], (*eth_p).MacSrc[5]);
	...
	//如果是IP协议，则继续解包；否则就不解了，直接走下一个包
		...
		//注意ntohs，从网络顺序(big endian) 主机顺序(little edian)的转换
		    ...
		    printf("IP source address is:%s\n", inet_ntoa(*(in_addr *) (*ip_p).ip_src.s_addr));
		    printf("IP destination address is:%s\n", inet_ntoa(*(in_addr *) (*ip_p).ip_dst.s_addr));
		...
		//嵌套循环,对TCP UDP ICMP三种协议进行解包
			...
			//下面进行tcp解包
			case 0x06:
			    fseek(fp, (*pkt_p).caplen - sizeof(struct EthernetPacket) - sizeof(struct ip), SEEK_CUR);
			    break;
			...
			case 0x11:
			    ...
			    fseek(fp, (*pkt_p).caplen - sizeof(struct EthernetPacket) - sizeof(struct ip) - sizeof(struct udphdr), SEEK_CUR);
			    break;
			...
			//void dump_icmp(char *packet, int offset);
			    dump_icmp(icmp_p, 0);
			    fseek(fp, (*pkt_p).caplen - sizeof(struct EthernetPacket) - sizeof(struct ip) - sizeof(struct icmphdr), SEEK_CUR);
		...
		case 0x0806:
		    ...
		    printf("ARP sender mac address: %02x:%02x:%02x:%02x:%02x:%02x\n",
			   (*arp_p).arp_sha[0], (*arp_p).arp_sha[1], (*arp_p).arp_sha[2], (*arp_p).arp_sha[3], (*arp_p).arp_sha[4], (*arp_p).arp_sha[5]);
		    printf("ARP sender IP address: %d.%d.%d.%d\n", (*arp_p).arp_spa[0], (*arp_p).arp_spa[1], (*arp_p).arp_spa[2], (*arp_p).arp_spa[3]);
		    printf("ARP target mac address: %02x:%02x:%02x:%02x:%02x:%02x\n",
			   (*arp_p).arp_tha[0], (*arp_p).arp_tha[1], (*arp_p).arp_tha[2], (*arp_p).arp_tha[3], (*arp_p).arp_tha[4], (*arp_p).arp_tha[5]);
		    printf("ARP target IP address: %d.%d.%d.%d\n", (*arp_p).arp_tpa[0], (*arp_p).arp_tpa[1], (*arp_p).arp_tpa[2], (*arp_p).arp_tpa[3]);
		    fseek(fp, (*pkt_p).caplen - sizeof(struct EthernetPacket) - sizeof(struct ether_arp), SEEK_CUR);
		    break;

//如果不是IP和arp协议，则直接进入下一个 包，不再解包        
		default:
		    printf("Other protocol , Not IP/ARP\n");
		    fseek(fp, (*pkt_p).caplen - sizeof(struct EthernetPacket), SEEK_CUR);
...
void
dump_icmp(struct icmpv4_hdr *packet, int offset)
{
